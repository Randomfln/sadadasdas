...
module.exports.execute = async(RegisterClient, message, args) => {
    ...
    if(!member.user.username.includes(settings.tag) && !member.user.discriminator.includes("0017") && !member.roles.cache.has(registerConfig.vip) && !member.roles.cache.has(registerConfig.booster)) return message.react(emojis.hata);
