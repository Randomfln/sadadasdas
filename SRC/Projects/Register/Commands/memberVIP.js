...
module.exports.execute = async(RegisterClient, message, args) => {
    ...
    if (!user) return registerfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}vip @Richârd/ID\``).sil(10000);
    ...
    if (!user) return registerfx.embedHata(message.author, message.channel, answer.hatalikullanim + ` \`${settings.prefix}vip <@Richârd/ID>\``)
